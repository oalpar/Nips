...
public:
      ...
      desired_false_positive_probability_(p.false_positive_probability)
  {
    ...
    salt_count_ = roundUp(salt_count_,8)/8;
    
    generate_unique_salt();
    for(int i = 0; i< salt_count_ ; i++){
      s_simpletab st_2;
      st_2.init(0);
      hashVec.push_back(st_2);
    }
    st_1.init(0);
 ...
 __m256i result;
	...
	std::cout<<"bit " << _mm256_extract_epi32(bit,1)<<std::endl;
	*/
	...
	for( int i =0; i< 8 ; i++){
	  ...
	  //	  std::cout<<"bit mask"<<arr2[i]<<" "<<bit_mask[arr2[i]]<<std::endl;
	 bit_table_[(int)arr1[i]/bits_per_char] |= bit_mask[arr2[i]];
	}
  ...
  inline virtual bool contains(int number, __m256i x1, int ch)
  {
    __m256i result;
    __m256i bit_index;
    __m256i bit;
	...
	_mm256_store_si256((__m256i *)arr2,bit);
	for( int i =0; i< 8 ; i++){
	  if ((bit_table_[arr1[i] / bits_per_char] & bit_mask[arr2[i]]) !=bit_mask[arr2[i]])
	    {
	      return false;
	    }
...
protected:
  ...
  inline virtual void compute_indices( __m256i& hash, __m256i& bit_index, __m256i& bit) const
  {
        bit_index = _mm256_and_si256 (hash, bitAnd);
 
    bit = _mm256_and_si256(bit_index, charAnd);
